...
function App() {
    ...
    useEffect(() => {
        ...
        takeTrack().then((data) => {
            dispatch({
                type: 'GET_ALL_OWNER_TRACK',
                allTrack: [...data.Ownertracks],
            })
            dispatch({ type: 'GET_ALL_Track', allTrack: [...data.tracks] })
        })
        takeFavouriteTrack().then((data) =>
            dispatch({ type: 'GET_ALL_FAVOURITE_TRACK', tracks: [...data] })
        )
        takeusers().then((data) =>
            dispatch({ type: 'GET_ALL_USERS', allUser: [...data] })
        )
    }, [])
    return (
        <div>
