...
function MusicList() {
    ...
    useEffect(() => {
        takeTrack().then((data) => {
            dispatch({
                type: 'GET_ALL_PublicTrack',
                allTrack: [...data.Publictracks],
            })
        })
    }, [])
    useEffect(() => {
        takeInputTrack(inputValue, false).then((data) =>
            dispatch({
                type: 'GET_ALL_PublicTrack',
                allTrack: [...data.tracks],
            })
        )
    }, [inputValue])

    const filteredTrack = React.useMemo(
        () =>
            publicTracks.filter((track) => {
                return track.name
                    .toLowerCase()
                    .includes(inputValue.toLowerCase())
            }),
        [inputValue, publicTracks]
    )
