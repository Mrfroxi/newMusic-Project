...
function MusicCardOwner({ music }) {
    ...
    const Setclosed = (e) => {
        e.stopPropagation()
        setisPublic(false)
        changeCloseTrack(false, music.id).then(() =>
            dispatch({ type: 'ADD_NEW_PUBLICTRACK', publicMusic: music })
        )
    }
    const SetclosedTrue = (e) => {
        e.stopPropagation()
        setisPublic(true)
        changeCloseTrack(true, music.id).then(() =>
            dispatch({ type: 'DELETE_TYPE_CL', musicId: music.id })
        )
    }
                ...
                <>
                    <div onClick={handlePlay} className={'music-card-cover'}>
                        ...
                        <div className="play-circle">
                            {isPublic ? (
                                <div
                                    className="LockBlock DontLock"
                                    onClick={SetclosedTrue}
                                >
                                    <Key className="Key" />
                                </div>
                            ) : (
